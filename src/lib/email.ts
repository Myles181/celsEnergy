...
export async function sendOtpEmail(otp: string) {
  ...
  try {
    ...
    // SMTP failed — print OTP to server console so login still works during dev/SMTP issues
    console.error("[CELS Admin] SMTP error:", err instanceof Error ? err.message : err);
    console.warn(`[CELS Admin] OTP (email failed — use this code): ${otp}`);
  }
}
